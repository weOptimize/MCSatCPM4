...
def simulate(arrayforsim, iterat):
    for i in range(len(arrayforsim)):        
        ...
        if arrayforsim[i] == 1:
            ...
            sim1_costs = MCS_CPM_RR(mydata, myriskreg, iterat)
            cashflows = []
            ...
            #print(cashflows)
            sim1_NPV = MCS_NPV(cashflows, iterat)
            #print(sim_NPV)
            ...
            median_npv = expected_value_extractor(sim1_NPV, iterat)
            for j in range(len(budgetting_confidence_policies)):
                ...
                survival_value = survival_value_extractor(sim1_costs, budgetting_confidence_policy, iterat)
            ...
            for j in range(len(budgetting_confidence_policies)): #MODIFICAR TODO ESTO CUANDO SE CAMBIE EL NUMERO DE POLICIES
                bdgtperproject_matrix[i][j]=0
                npvperproject_matrix[i][j]=0
    return(mcs_costs)
...
mcs_costs1 = simulate(candidatearray,100)

# copy the array with all MCS results
df0 = pd.DataFrame(data=mcs_costs1).T
df0.rename(columns={0:"P01", 1:"P02", 2:"P03", 3:"P04", 4:"P05", 5:"P06", 6:"P07", 7:"P08", 8:"P09", 9:"P10"}, inplace=True)
correlation_matrix0 = df0.corr()

# this function calculates the npv of each project and then uses the maximizer function to obtain promising solutions (hall of fame)
for i in range(len(budgetting_confidence_policies)):
    ...
    maxbdgt = 3800
    #run maximize function that returns a hall of fame of promising portfolios
...
#project the results of the hall of fame into a new array (the new array contains a zero only if the project was never selected in any array of the hall of fame)
Proj_for_sim2 = [max(col) for col in zip(*hall_of_fame)]
print(Proj_for_sim2)

#simulate all ther projects from the hall of fame and store the results in a new array
mcs_costs2 = simulate(Proj_for_sim2,200)
print(mcs_costs2)

#check the parameters of beta distribution for each of the mcs_results
betaparams = []
for i in range(nrcandidates):
    f = Fitter(mcs_costs2[i], distributions=['beta'])
    f.fit()
...
#extract the sixth portfolio included in array portfolio_results  --- CHANGED TO FIRST ONE WHILE WE DO ONLY ONE
...
#make sure no legend appears in the next plot REACTIVAR ESTA AL FINAL!!!! (todas lineas ok)
#plt.figure(12)
