...
for j in range(iterations):
    ...
    bdgtperproject_matrix = np.round(mcs_results1[0, :, j], 2)
    print("bdgtperproject_matrix: ", bdgtperproject_matrix)
    # extract second iteration of mcs_results1 to get the NPV of each project and store it in npvperproject_matrix
    npvperproject_matrix = np.round(mcs_results1[1, :, j], 2)-np.round(mcs_results1[0, :, j], 2)
    print("npvperproject_matrix: ", npvperproject_matrix)
    # define the budget constraint
    maxbdgt = 10800
    ...
    # assign the result from projectselection to the variable solutions
    NPVs.append(p.solution.get_objective_value())

    # calculate the average value of the solutions
    average = np.mean(NPVs)
    print("Array of NPVs of the iterated Solutions:", NPVs)
    print("Average value of NPV's achieved: ", average)

    # assign the result from projectselection to the variable solutions
    solutions.append(p.solution.get_values())

    # assign the budget calculation for each solution to the variable budgetedcosts
    portfolio_budgets.append(p.solution.get_values() @ bdgtperproject_matrix)

    print("solutions: ", solutions)
    print("portfolio_budgets: ", portfolio_budgets)
